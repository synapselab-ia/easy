...
D-035 / Dashboard + Reports core redesign is the authorized product initiative. `DR-01` (product contract/canonical documentation), `DR-02` (canonical Dashboard read-model), `DR-03` (primary Dashboard KPI row), `DR-04` (`Precisa de atenção` action center) and `DR-05` (compact carteira aging) are complete. **`DR-06 — recent registrations + quick actions` is the sole current executable item.** Later `DR-07…DR-09` items remain queued/not current and must not be bundled.
