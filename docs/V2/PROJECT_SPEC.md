...
Accepted DR-08 implementation:

- the primary Reports row and the PDF summary both use `Vendas`, `Recebimentos`, `Movimento líquido`, `Em aberto no fim`, while order/item counts remain supporting context;
- `FinancialReport.comparison` now carries previous-period `periodNet` and its percentage change so comparison remains centralized rather than recalculated by the UI;
- each KPI exposes the actual immediately preceding equal-length comparison range;
- product investigation adds accent/case-insensitive product-name search, historical-category filtering and sorting by sales, quantity or order count over canonical report rows;
- reseller investigation adds accent/case-insensitive name search and sorting by sales, receipts, report-end open debt or order count;
- `FinancialReport.resellerAnalysis` owns selected-period reseller-sales Pareto/concentration and top positive report-end `openDebt` rows;
- the existing Pareto presentation component receives canonical report data; the removed Dashboard-only `90/180/360` calculation path is not resurrected;
- report-end positive balance is explicitly described as an open receivable position and not automatically as `inadimplência`;
- screen and PDF continue to consume the same `FinancialReport` object; no alternate accounting model, mutation path or persistence contract was introduced.
...
- primary screen KPIs are `Vendas`, `Recebimentos`, `Movimento líquido`, `Em aberto no fim`; orders/items are supporting sales context;
- the same primary hierarchy is used by the report PDF summary;
- the actual immediately preceding equal-length comparison range is visible on primary metrics;
- canonical `FinancialReport.comparison` now includes previous `periodNet` and `periodNetChangePercent`;
- product analysis supports accent/case-insensitive name search, historical-category filtering and sorting by sales, quantity or orders;
- reseller analysis supports accent/case-insensitive name search and sorting by sales, receipts, report-end open debt or orders;
- canonical `FinancialReport.resellerAnalysis` owns selected-period reseller-sales Pareto/concentration plus top report-end positive open balances;
- the existing Pareto visual consumes canonical report data rather than the removed Dashboard `90/180/360` path;
- positive report-end balance is labeled as open balance and explicitly not treated as automatic inadimplência;
- Dashboard operational content, mutation/accounting semantics, database/schema, Supabase/RPC/Auth/RLS, recovery and deployment paths are unchanged.

The first D-019 run/job `33162503781` / `98820074403` was not accepted: ESLint had 0 errors / 108 warnings; domain DR-08 tests 8/8 and PDF tests 3/3 passed; the new `ReportsPage` tests were 2/3 and the full Vitest suite stopped at 308/309 because one new assertion ambiguously queried the legitimate duplicated text `Recebimentos`. Playwright/build did not run and nothing was integrated. Only that test targeting was made precise; product behavior was unchanged and the full gate was rerun.
...
Run a bounded final pass across desktop/mobile, empty/loading states, accessibility, wording, deep-link behavior and performance. Fix only evidence-backed defects found by that acceptance pass; do not use it to invent unrelated features or reopen accepted accounting/product semantics.
