...
D-035 / Dashboard + Reports core redesign is the authorized product initiative. `DR-01` (product contract/canonical documentation), `DR-02` (canonical Dashboard read-model), `DR-03` (primary Dashboard KPI row), `DR-04` (`Precisa de atenção` action center), `DR-05` (compact carteira aging) and `DR-06` (recent registrations + quick actions) are complete. **`DR-07 — remove Dashboard Performance block + contextual Reports handoff` is the sole current executable item.** Later `DR-08…DR-09` items remain queued/not current and must not be bundled.
...
DR-06 added only the prepared recent-registration projection and existing-route quick actions. `RecentRegistrations` preserves the snapshot-provided order and reversal exclusion, distinguishes transaction type, value and reseller, disambiguates occurrence date from registration date when needed, and navigates to existing reseller history. `DashboardQuickActions` routes the three accepted intents into the existing transaction form. PR #122 passed full D-019 and exact validated/integrated tree equivalence; `PerformanceAnalysisSection` remains unchanged for DR-07.
